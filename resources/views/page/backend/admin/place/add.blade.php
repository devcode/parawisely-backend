...
<div class="title mb-3">
    <div class="row">
        <div class="col-md-6">
            <h3>Tambah {{ $title }}</h3>
            <small class="form-text text-muted">Klick tempat kamu di map agar data bisa terkirim <span
            ...
            <div class="col-md-5">
                <div class="card shadow">
                    <div class="card-body">
                        <div id="map"></div>
                        <div class='pointer'>
                            << Klik untuk mencari tempat</div> <div class="form-group mt-2">
                                <label for="address" class="control-label">Alamat <span
                                        class="text-danger">*</span></label>
                                <textarea id="address"
                                    class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}"
                                    name="address" rows="6">{{ old('address') }}</textarea>
                                {!! $errors->first('address', '<span class="invalid-feedback"
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <div class="form-group">
                            <label for="name_palce" class="control-label">Nama Tempat<span class="text-danger">
                                    *</span></label>
                            <input id="name_place" type="text"
                                class="form-control @error('name_place') is-invalid @enderror" name="name_place"
                                value="{{ old('name_place') }}" autocomplete="off">
                            @error('name_place')
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="address" class="control-label">Tipe Tempat<span class="text-danger">
                                            *</span></label>
                                    <select name="type_place" id="type"
                                        class="form-control @error('type_place') is-invalid @enderror">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="island" class="control-label">Pulau<span class="text-danger">
                                            *</span></label>
                                    <select name="island" id="island"
                                        class="form-control @error('island') is-invalid @enderror">
                                        <option disabled selected>--PILIH PULAU--</option>
                                        @foreach ($dataIsland as $row)
                                        <option value="{{ $row->id }}">{{ $row->name }}</option>
                                        @endforeach
                                    </select>
                                    {!! $errors->first('island', '<span class="invalid-feedback"
                                        role="alert">:message</span>') !!}
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="provinsi" class="control-label">Provinsi<span class="text-danger">
                                            *</span></label>
                                    <select class="custom-select @error('propinsi') is-invalid @enderror"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="kabupaten" class="control-label">Kab / Kota</label><span
                        ...
                        <div class="form-group">
                            <label for="description" class="control-label">Deskripsi<span class="text-danger">
                                    *</span></label>
                            <textarea id="description"
                                class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}"
                                name="description" rows="4">{{ old('description') }}</textarea>
                            {!! $errors->first('description', '<span class="invalid-feedback"
                                role="alert">:message</span>') !!}
                        </div>
                        <div class="form-group">
...
<script>
    ...
    var map = L.map('map').setView(mapCenter, {
        {
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
            ...
            success: function (json) {
                if (json.code == 200) {
                    for (i = 0; i < Object.keys(json.data).length; i++) {
                        $('#propinsi').append($('<option>').text(json.data[i].name).attr('value',
                            json.data[i].name).attr('data-prov', json.data[i].id));
                    }
                } else {
                    $('#kabupaten').append($('<option>').text('Data tidak di temukan').attr('value',
                        'Data tidak di temukan').attr('data-prov', json.data[i].id));
                }
            }
        });
        $("#propinsi").change(function () {
            ...
            $.ajax({
                url: 'https://x.rajaapi.com/MeP7c5ne' + window.return_first +
                    '/m/wilayah/kabupaten',
